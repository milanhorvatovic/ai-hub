...
- Run `./venv/bin/pytest -q` before pushing. Markdown is formatted with Prettier (`proseWrap: never`) per `.prettierrc.json` — author prose as one line per paragraph and let it wrap. **This is enforced:** the `lint` workflow runs `npm run format:check` over every tracked `.md` on each pull request, and a hard-wrapped paragraph fails it. Run `npm ci` once to install the pinned Prettier, then `npm run format` to fix formatting locally. Generated files stay out of the gate via `.prettierignore` — today that is release-please's per-skill changelogs, which are rewritten on every release.
