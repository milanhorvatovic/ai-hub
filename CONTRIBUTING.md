...
- `tests/skills/context-cost-baseline.json` — the recorded discovery / router / load bytes of every skill, and the `metadata.version` they were measured at. The suite fails when the record no longer describes the tree, so a change in what a skill costs to load lands as a reviewable diff; refresh with `./venv/bin/python -m tests.support.context_cost`. The size itself is not gated — only the record going stale is — and the one tolerated drift is release-please's own version rewrite, which the guard absorbs arithmetically so the release PR stays green on a branch nothing can refresh.
