...
Every action reference is pinned to a full commit SHA, with the version as a trailing comment on the same `uses:` line (`uses: actions/checkout@de0fac2e… # v6.0.2`) — the one position Dependabot rewrites when it bumps a pin, so the comment stays true instead of rotting on the line above. First-party `actions/*` are held to the same bar. The `action-pins` check enforces this on every pull request, running the checker from the base branch (the same trust model as the title gate) against the PR's workflows as data; `tests/repo/test_action_pins.py` holds the rule against the tree and cross-checks the text-level scanner against a real YAML parse.
