...
- **At least two runs per probe per model.** One run is an anecdote. A trait counts as _observed_ only when it appears in the majority of runs — and each run starts in a fresh model context with a reset copy of every mutable fixture, because runs that share conversation history or leftover file and tool state are correlated, and the majority rule then corroborates the contamination rather than the trait.
...
- **The draft is introspection-grade, not a deliverable.** The source flatters itself like any self-report: its rules enter the extraction table as hypotheses tagged `self-reported`, and each one needs a source task probe corroborating that the source actually works that way before it is encoded — a matching target baseline failure proves the gap is real, not that the source closes it. Everything that survives still goes through the full stage-6 loop. Never ship the draft as-is.
