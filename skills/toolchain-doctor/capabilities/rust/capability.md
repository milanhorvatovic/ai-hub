...
5. **Component provisioning** — whether the toolchain the job installs carries `rustfmt` and `clippy`. Both are optional `rustup` components, so a minimal profile has neither, and `cargo fmt --check` then exits on a missing component before it reads a line of the crate. Resolve it from what the workflow installs: a `components` list, a profile, or a `rust-toolchain.toml` the action honours. Where the installing step's defaults cannot be established from this repository — an action whose profile is its own business and moves between releases — that is `unknown`, not a pass, and the check is worth naming since the scaffold provisions both components for exactly this reason.
6. **Toolchain pinning** — whether a `rust-toolchain.toml` exists and what channel it names. Report the channel with whatever explains it: a `nightly` pin beside a nightly-only feature explains itself, and one with no such feature in sight is worth a question. Neither is graded here — the scan reports both the same way.
...
- **Lints riding a floating toolchain.** `cargo fmt` and `cargo clippy` ship with the toolchain, so their version is the channel's. A crate on `stable` therefore takes new lints the day they release, and a green pull request can redden on a change nobody made. That is `floating` like any other unfixed version, since the state is the same whoever chose it; what is unusual here is that the state is the ecosystem's own default rather than an oversight, which the finding says out loud. Say it once, and let the prescription be that the lever exists and that pinning the channel costs the new lints, rather than a recommendation to pull it.

The floor asks a project to respect the MSRV `Cargo.toml` declares and asks nothing about declaring one, so an absent `rust-version` is not a `gap` and carries no grade at all. The python lane reads the other way for a reason written into its own floor, which does ask for a declared interpreter and calls its absence a real finding; the difference lives in the floors rather than in the lanes, and carrying one lane's rule across to the other is how an absent `rust-version` acquires a finding nothing asked for. A declared MSRV carries no grade either, for existing being what was asked. Report the value among the scan's facts. What the audit does say is when CI does not test it: a `rust-version` nobody builds against is a promise with no evidence behind it. That is a `wiring` finding — the declaration exists and nothing exercises it — and the prescription is a CI matrix entry rather than a change to the declaration.
