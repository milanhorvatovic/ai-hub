...
- **Package selection mistaken for target selection.** `--all-targets` widens which _targets_ of the selected packages are linted; it does not widen which packages are selected. In a workspace the selection comes from `--workspace`, `-p`, or — when neither is given — `default-members`, so a workspace that sets `default-members` can run `clippy --all-targets` in CI and never lint the members left out of that list. Read the selection and the target flags as two separate facts, and grade the lint row against the members actually covered rather than against the flags that look thorough. A workspace whose CI omits `--workspace` while `default-members` is set is a `wiring` finding naming the uncovered members, not a satisfied row.
