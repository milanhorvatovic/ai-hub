...
# One section per extensionless path the inventory classified as shell — repeat
# this block for each. EditorConfig matches on path patterns, so a `[{a,b}]`
# brace list would fold them into one glob and split a path that itself holds a
# comma, and `* ? [ ] { }` in a path are glob metacharacters a literal match must
# backslash-escape. Each is an exact path, never a `.githooks/*` wildcard: that
# wildcard is the trap this section avoids one directory in — a hook that is not
...
[<one extensionless path the inventory returned, EditorConfig metacharacters backslash-escaped>]
indent_style = <the project's existing style>
indent_size = <the project's existing width>
switch_case_indent = <true if the scripts indent case bodies, false if not>
```
...
What was measured here, against `shellcheck` 0.11: `sh`, `bash`, `dash`, `ksh`, `oksh`, `bats`, and `busybox sh` named by a direct path all exit clean; `ash` exits non-zero with `SC2187` until the file carries `# shellcheck shell=dash`, after which it exits clean; `mksh` raises `SC1008`, `zsh` raises `SC1071`, and `busybox sh` routed through `env` raises `SC1008`. `shfmt` was **not** measured — it is not installed where this was written — so its set is deliberately left for the adopter to establish against the pinned version rather than asserted from memory, and by the same test the trap above prescribes: run each shebang name the inventory produces against that `shfmt` and keep the ones it accepts, mapping each to a grammar it takes. Reading the set off `--help` is the mistake that paragraph warns of — help prints the parser labels, and copying `posix` into an alternation matched against shebang names drops the `#!/bin/sh` and `#!/bin/dash` scripts it stands for. A reviewer of this template reported that current `shfmt` handles `zsh`, which is plausible and which nothing here can confirm; that is precisely why the value is a placeholder and not a list.
...
Tracked files only, and the reason is the CI step this feeds. An untracked script exists on the machine that ran the discovery and nowhere else, so copying it into a workflow's file list produces a job that fails on a path absent from the checkout — a scaffold that works for its author and for no one else. `git ls-files` is also what makes the shebang pass finite: it walks what the repository tracks rather than everything under the working directory, so untracked scratch in the working tree never reaches the `head` check. What it does not do is skip a tracked file for matching an ignore rule — `git ls-files` lists a tracked path whether or not `.gitignore` also names it — so the common build and vendor trees are kept out by the explicit pathspec exclusions above, not by the walk, and a repository that tracks generated output under some other directory adds that directory to those exclusions itself.
