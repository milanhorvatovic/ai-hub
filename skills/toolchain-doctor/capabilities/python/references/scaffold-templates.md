...
This closes the floor row on its own: the row grades checker coverage, and a config naming the package under a declared interpreter is coverage. No enforcement setting is scaffolded, and the reason is a gap in what the tools can express. The floor asks for annotations on the **public** surface and explicitly lets obvious internal helpers go untyped; `mypy` has no setting scoped that way. `disallow_untyped_defs` is the nearest lever and it reaches every definition in the package, so scaffolding it to close a public-surface finding would gate legacy internals the floor never asked about — trading an under-enforcing default for an over-enforcing one.

So the audit reads the enforcement settings the repository declares and reports them beside the row rather than as part of it, naming `disallow_untyped_defs` as the available lever together with what it costs. Write it when the user accepts that scope — and because it is beside the row rather than in it, a repository scaffolded from a missing-checker finding re-audits clean whether or not they do.
...
Not every manager ships a setup action, and the ones that do not — `pipenv`, `hatch`, `pip-tools` — take the other shape that paragraph names, a pinned installer step in place of the `uses:` block above. There is no `<that manager's pinned setup action>` to fill for these, so a scaffold that only knew the `uses:` shape could not emit a complete file for them; this is the branch it takes instead. It installs the manager through `pipx` — present on the standard runners and isolating the tool from the project's own environment — rather than a bare install into the runner's own interpreter, which the audit grades as installing past the environment manager; and it pins the manager exactly so the version is fixed rather than resolved to the latest release on the day CI runs, the same fixity the floor grades and a `floating` finding if the `==` is dropped:

```yaml
      - name: bootstrap <the project's environment manager>
        run: pipx install <the manager>==<the exact version, not a floating default>
```
...
One home, referenced by the rest — and the home is the one the project already uses for metadata, not a new one this scaffold introduces. A project on PEP 621 declares it in `pyproject.toml`'s `[project]`; one whose metadata lives in `setup.cfg` declares `python_requires` under `[options]`; one still using `setup.py` passes `python_requires` to `setup()`. Adding a `[project]` table to a project that owns its metadata in `setup.cfg` either does nothing, because the tool reads the other file, or stands up a second source that disagrees with the first — the interpreter-version gap unclosed either way. Read the owner off the scan and write the declaration there. Reserve the TOML shape below for a project already on PEP 621 — one whose `[project]` table exists and already carries the `name` and the `version` that table requires: a static `version`, or `version` listed in `dynamic` for a setuptools-scm or hatch-vcs project that derives it from the VCS, so `requires-python` only joins them. It is not a shape to drop into a repository with no metadata file at all: PEP 621 makes `name` mandatory the moment `[project]` appears, and `version` too unless it is deferred through `dynamic`, so a table holding `requires-python` alone is invalid and breaks the packaging tools that read it. A repository that is not a package keeps its interpreter version in `.python-version` instead; one that is meant to become a package gets a complete `[project]` table, with the `name` and a `version` — static or declared `dynamic` — asked for rather than a bare `requires-python`.

```toml
# pyproject.toml — a project already on PEP 621 (its [project] carries name and a static or dynamic version)
[project]
