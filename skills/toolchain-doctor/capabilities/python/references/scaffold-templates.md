...
One home, referenced by the rest — and the home is the one the project already uses for metadata, not a new one this scaffold introduces. A project on PEP 621 declares it in `pyproject.toml`'s `[project]`; one whose metadata lives in `setup.cfg` declares `python_requires` under `[options]`; one still using `setup.py` passes `python_requires` to `setup()`. Adding a `[project]` table to a project that owns its metadata in `setup.cfg` either does nothing, because the tool reads the other file, or stands up a second source that disagrees with the first — the interpreter-version gap unclosed either way. Read the owner off the scan and write the declaration there. Reserve the TOML shape below for a project already on PEP 621 — one whose `[project]` table exists and already carries the `name` and `version` that table requires, so `requires-python` only joins them. It is not a shape to drop into a repository with no metadata file at all: PEP 621 makes `name` and `version` mandatory the moment `[project]` appears, so a table holding `requires-python` alone is invalid and breaks the packaging tools that read it. A repository that is not a package keeps its interpreter version in `.python-version` instead; one that is meant to become a package gets a complete `[project]` table, with the name and version asked for rather than a bare `requires-python`.

```toml
# pyproject.toml — a project already on PEP 621 (its [project] carries name and version)
[project]
