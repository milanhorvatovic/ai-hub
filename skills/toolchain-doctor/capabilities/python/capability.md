...
2. **Type checker** — `mypy` or `pyright`, and the scope it resolves to, read from the config **and** the invocation together with the checker's own defaults applied. Neither half decides alone, and neither tool has the failure this row was first written against: `mypy` with no target in either place exits with an error rather than passing, and `pyright` with no `include` defaults to the project directory rather than to nothing. So the report is the effective scope, and the finding is scope **narrower than the code it appears to cover** — a checker pointed at one package in a repository holding six, which passes honestly while six-sevenths of the codebase has never been typed. Name what is outside it.
   ...
   Scope is only half the row. The floor asks for type hints on the public surface — module-level functions, class methods, dataclass fields — and a checker traversing every file still reports nothing about that at its default settings: `mypy` permits untyped definitions unless told otherwise, and `pyright`'s `standard` mode does not require complete public annotations. A repository can therefore run a type checker over all of its code, pass, and carry no annotations at all, which is the row graded satisfied while nothing it exists for is true. So the row does not come back **satisfied** on coverage alone. Coverage is what the scan can establish; whether the public surface is actually annotated is not, because establishing it would mean reading the code, which this skill does not do. Where an enforcing setting is declared — `disallow_untyped_defs`, `pyright`'s missing-annotation diagnostics — say what it establishes rather than what the floor asks for: those settings require annotations on **function and method definitions**, which is two of the three things the floor names. Field annotation is not something they check. In stdlib dataclasses that gap is narrow, because an attribute without an annotation is not a field at all; in the frameworks that build on the same idea it is wider. So the row reports the def-level enforcement as established and the field portion as unverified, rather than reading a setting that covers most of a requirement as covering all of it. Where none is, the row is `unknown`: coverage established, annotation unverified, with the setting named as the way to make it verifiable.

   `unknown` rather than `gap` because nothing here says the repository is failing — it may be fully annotated by habit — and `unknown` rather than satisfied because the audit has not seen that and should not imply it has. This is what that grade was added for. Note also that neither enforcing setting is scoped the way the floor is: both reach every internal helper, which the floor explicitly leaves alone, so adopting one is a recommendation with a stated cost rather than something the minimal scaffold writes on the user's behalf.

   Establish enforcement as a reported fact, the way everything else here is established — from what the repository declares. `disallow_untyped_defs` for `mypy`, the missing-parameter and missing-return diagnostics for `pyright`: present and enabled is enforcement, absent is not, and a mode name is not evidence either way. Where the effective settings cannot be resolved from the configuration — an `extends` chain leaving the repository, a checker invoked through a wrapper the scan cannot read — that is `unknown` with the reason named. It is not an invitation to run the checker: this skill does not execute the tools it audits, and a row resolved by running one would be answering a code-quality question to settle a setup question.
