...
- **`ruff` rule selection that excludes the reason it was adopted.** A config selecting only `E` and `F` turns `ruff` into `pyflakes`; that is a legitimate choice and worth surfacing as a `decision` when it looks deliberate, and as an observation when the config reads like a default nobody revisited.
