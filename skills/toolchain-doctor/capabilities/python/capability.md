...
- **`ruff` rule selection that excludes the reason it was adopted.** A config selecting only `E` and `F` is running pycodestyle's error family beside Pyflakes — roughly what Flake8 gives without plugins. The floor asks for `ruff` to lint and says nothing about which families, so that selection satisfies the row and earns no grade: report the active families among the scan's facts, where a maintainer can see whether the config reads deliberate or like a default nobody revisited, and leave the audit's findings for rows something actually failed.
