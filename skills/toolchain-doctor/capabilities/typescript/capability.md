...
- **A tool invoked without being declared.** `npx eslint` resolves the project's own installed binary when `eslint` is a declared dependency — that is the resolution order, and a step written this way in a repository that declares the package is pinned, not floating. The `floating` finding belongs to the case where nothing in the tree declares the package: `npx` then fetches whatever is current, and a rule added upstream arrives as a red build on an unrelated change. Judge the declaration, never the spelling of the command. Where the package is declared, the improvement worth naming is `npx --no-install`, which restricts resolution to the local copy and fails loudly when dependencies have not been installed rather than silently reaching past them; propose it as a recommendation, not as a finding.
