...
- **A tool invoked without being declared, or declared without being fixed.** These are two steps and both must pass. `npx eslint` runs the project's own installed copy — but the condition is that the dependency is _installed_, not merely declared: with `node_modules` absent, `npx` fetches a temporary copy into its cache, so a job that never ran its install reaches past the committed lock to whatever resolves today. The command's spelling still proves nothing either way, and a rule that reads `npx` as floating is wrong about every repository that installs its declared tools — but the fix is not the declaration alone. Judge fixity per `../../references/modes.md` — an exact constraint, or a tracked lock a completed install consumed — and grade `floating` when neither holds, whether the invocation says `npx` or not; a caret range with no committed lock, and a lock the CI job never installs from, are the same finding. Where the package is declared, `npx --no-install` is worth naming as a recommendation: it restricts resolution to the local copy and fails loudly when dependencies have not been installed rather than silently reaching past them.
