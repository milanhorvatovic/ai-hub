...
- **The repo's own declaration outranks the floor.** A repository that pins a language version below the floor has declined that row, and the audit records the decline and moves on rather than re-arguing it. Two cases that look identical are not: picking either tool in an _or_ pair, or switching off a rule with the reason the floor asked for, satisfies the row outright and earns no grade at all — grading those is grading a repository for doing what was asked. What the rule protects is a declaration someone wrote down, so it does not reach the cases where nobody chose: a floor row nothing satisfies, a version nothing fixes, and a declaration that contradicts itself — a tool configured but never run, two formatters fighting over the same files, a version pinned in one place and floated in another — are all findings, and each says which of those it is.
