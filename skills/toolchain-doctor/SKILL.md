...
- **The repo's own declaration outranks the floor.** A repository that pins a lower language version, disables a rule, or picks the other tool in a pair has made a decision; the audit records it as a decision and moves on. It is a finding only when the declaration contradicts itself — a tool configured but never run, two formatters fighting over the same files, a version pinned in one place and floated in another.
