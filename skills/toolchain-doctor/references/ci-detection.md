...
So a hook backed by a repository-owned invocation grades "runs, weakly": it covers what that invocation covers and not the merge. A hook backed only by instructions is reported as declared with execution unestablished, which is an honest thing to hand a maintainer — they know whether their team ran it, and the audit does not. 4. **Editor and IDE config** — `.vscode/settings.json` and friends. This is _not_ execution. A formatter enabled on save in one editor's settings is a convenience for the people using that editor, and reporting it as wiring would be the most flattering possible reading of the evidence.
