...
- **Runs, weakly**: cannot fail a change that would break it. Either the failure is swallowed — `|| true`, `continue-on-error: true`, a step marked non-blocking — or it never fires before the change lands: hook-only, schedule-only, or a trigger scoped so that the first run happens after the merge. A job that fires only on pushes to the default branch belongs here — it does run, and it runs after review rather than during it, so a break is caught once already merged.

Read that as behaviour, not as the absence of an event name, or this bullet contradicts the one above it. A workflow triggered by `push` with no branch filter fires on every feature branch and reports against the candidate head, which is a check before the merge and grades **Runs**; the same workflow filtered to the default branch alone is the weak case. What a push-only arrangement does not cover is a pull request from a fork, because no push to this repository happens for one. Where the repository takes fork contributions, say so with the finding rather than averaging it into a single grade: the invocation is strong for the branches it sees and absent for a whole class of change, and a reader who contributes by fork needs that stated rather than summarized.

One trigger is graded by its execution context rather than by whether it fires before the merge, because the context is the whole point. `pull_request_target` runs in the base repository's privileged environment, with its secrets and a write-capable token in reach, so a job on it either checks out the base — where it faithfully lints code that does not contain the change — or checks out the head, where it runs a contributor's code with the repository's credentials available. The first runs, and not over the change, which is the weak grade. The second is the pattern usually called a pwn request, and the audit names it as a finding about the workflow rather than counting it as the tool running at all. Grade any `pull_request_target` invocation weakly, and where it reaches the head, say why that is worse than not running.

That the ordinary `pull_request` trigger is the safe place to lint a contributor's change is a property of the source, not of the event name — the reason to reach for `pull_request_target` in the first place, and the reason not to grade against it. A `pull_request` from a fork arrives without the repository's secrets and with a read-only token, which is exactly what makes running untrusted code on it safe; a `pull_request` from a branch of the same repository carries the same credentials as any in-repo job. So resolve which source a job faces before trusting the trigger, and where the scan cannot tell a fork from a same-repo branch, say so rather than assuming the harmless case.
