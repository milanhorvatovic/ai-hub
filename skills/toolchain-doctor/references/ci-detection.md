...
- **Runs**: fires on pull requests, with a failing exit status that fails the job, **and actually reaches the change**. The last clause is the one most easily assumed. A workflow `paths` filter, a job or step `if:` condition, or a matrix leg that excludes the files in question can all leave a pull-request-triggered job green without the tool ever seeing what changed. `pull_request_target` deserves naming separately, and it never earns the strong grade. It runs in the base repository's privileged context, with secrets and a write-capable token available, so a job on that trigger either checks out the base — in which case it faithfully lints code that does not contain the change — or checks out the head, in which case it is executing a contributor's code with the repository's credentials in reach. The first is weak wiring. The second is the pattern usually called a pwn request, and the audit reports it as a finding about the workflow rather than as evidence the tool runs.

So: grade any `pull_request_target` invocation weakly, and where it reaches the head, say why that is worse than not running at all. Linting a change belongs on `pull_request`, which runs the contributor's code without the credentials. Otherwise confirm the trigger's scope covers the tool's files, and grade weakly where it cannot be confirmed, naming what was missing.
