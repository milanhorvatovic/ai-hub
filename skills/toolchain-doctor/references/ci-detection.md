...
3. **Hook configuration** — `.pre-commit-config.yaml`, `.husky/`, `lefthook.yml`, `.githooks/`. A tracked hook file is a **declaration**, not an execution: `.githooks/` does nothing until `core.hooksPath` points at it, and a pre-commit config does nothing until the hook is installed, and neither of those is in the repository. Reading presence as running would merge the two columns this whole reference exists to keep apart. Look for evidence of activation the repository itself carries — a setup script or documented step that sets the path, an installer invoked in CI — and where none is found, report the hook as declared and not established to run. Where activation is evidenced, a hook-only tool grades "runs, weakly": it covers the machines that installed it and not the merge.
