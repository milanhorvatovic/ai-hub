...
| rust | `Cargo.toml` | `*.rs` |
| bash | none — shell has no manifest | `*.sh`, `*.bash`, `*.bats`, extensionless files whose first line is a shell shebang, tracked `.husky/` hooks other than the generated `.husky/_/` wrappers — husky supplies the interpreter, so its hooks carry neither shebang nor extension and an evidence rule reading only those two would skip a repository whose shell is all hooks — and embedded shell **whose interpreter resolves to a shell**: a multi-line `run:` block whose step does not select another shell, a `Makefile` recipe whose `SHELL` names a shell — or, where `SHELL` is unset, one whose build runs on Unix rather than the Windows where GNU Make reaches for `cmd` through `COMSPEC` — a Dockerfile `RUN` in shell form on an image whose default is one |
...
No read follows a symlink out of the checkout, and no scaffold is written through one — a rule that holds mode-wide, not only for the extensionless script that first raised it. A repository can track a link — an extensionless script, but equally a `pyproject.toml` or a `.shellcheckrc` — whose target sits outside the working tree, so a scan that opened it would read an arbitrary file off the runner and could disclose it, and a scaffold written onto an existing link would overwrite whatever it points at, past the confirmation the write already asked for. Resolve a path before trusting it: on the read side skip a tracked link by its git file mode or an explicit check, the way the bash scaffold's discovery does before handing a path to a linter; on the write side refuse to write through a link and create the file directly. This holds at stage 0, in every lane's declaration and source scan, and at every scaffold destination.
...
A language with manifest evidence is present, once that evidence has cleared the corroboration the two rules above demand — a bare `package.json` or `requirements*.txt` is exactly the case they qualify, so "has a manifest" is not the test where those two files are concerned; "has a manifest the corroboration rules accept" is. A language with extension evidence alone is present too, and worth naming as such in the report — a repository with twelve `.py` files and no `pyproject.toml` is exactly the case where the audit has something to say. A language with neither is absent, and the report says which languages were looked for, not only which were found: a reader cannot tell "no Rust here" from "Rust was never checked" unless the report distinguishes them.
...
2. **Draft from the capability's own scaffold templates** — each language capability carries them beside its capability file — filled in against what the scan found: the repository's declared language version, its existing tool choices, its CI shape. The config files a scaffold writes — an `.editorconfig`, a `tsconfig.json`, a lint config — are provider-neutral and drop in anywhere. The CI-step templates are not: they are written for GitHub Actions, which is the one CI syntax these templates spell out, while the scan reads GitLab, CircleCI, Azure Pipelines, and Jenkins as readily. Where a wiring finding lands on a repository whose CI is one of those, the scaffold does not emit GitHub YAML into it — it hands over the job's _requirements_, the trigger that runs before the merge, the permission floor, and the exact commands the step must run, for the user to place in their provider's syntax, and says that is what it is doing. Inventing a provider's YAML the templates do not carry would be the scaffold guessing, which the audit's own honesty rules forbid.
