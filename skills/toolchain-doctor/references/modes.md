...
| rust | `Cargo.toml` | `*.rs` |
| bash | none — shell has no manifest | `*.sh`, `*.bash`, `*.bats`, extensionless files whose first line is a shell shebang, and embedded shell **whose interpreter resolves to a shell**: a multi-line `run:` block whose step does not select another shell, a `Makefile` recipe where `SHELL` has not been reassigned, a Dockerfile `RUN` in shell form on an image whose default is one |
