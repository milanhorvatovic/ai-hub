...
| rust | `Cargo.toml` | `*.rs` |
| bash | none — shell has no manifest | `*.sh`, `*.bash`, `*.bats`, extensionless files whose first line is a shell shebang, and embedded shell: multi-line `run:` blocks in workflows, `Makefile` recipes, Dockerfile shell-form `RUN` instructions |
