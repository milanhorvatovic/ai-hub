...
- `assets/configs/prettierrc.json` — markdown overrides set `proseWrap: "never"`; top-level `embeddedLanguageFormatting: "off"` so prettier never reformats code inside fenced blocks (keeps illustrative `yaml` / `json` examples — e.g. GitHub-Actions `${{ }}` snippets — byte-for-byte).
