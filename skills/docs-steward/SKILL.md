...
4. **Missing tool** when the baseline declares it → `MISSING` event with the install hint and exit 3; **never** auto-install. The agent decides what to do next.
...
The repo's own config always wins when present; `--baseline FILE` skips both detection and fallback. The shipped defaults align with the no-hard-wrap + compact-tables preference:
