...
1. **Probe + selection order.** A declared config selects its family (`.markdownlint*` → markdownlint, `.prettierrc` → prettier, …). With no config, the fallback favors **prettier** (`prettier` → `markdownlint-cli2` → `markdownlint` → `mdformat` → `dprint` → `remark`) so a config-less repo gets consistent formatting including `proseWrap: never`. In **audit** mode a complementary **markdownlint lint pass** then runs alongside any non-markdownlint formatter (lint-only, bundled config) so the semantic `MD###` rules (MD040, MD036, MD033, MD034, …) are still reported even though prettier owns formatting. `yamllint` is probed independently for the `md-audit-frontmatter` pipeline — it never participates in markdown formatter selection.
...
- `assets/configs/prettierrc.json` — markdown overrides set `proseWrap: "never"`; `embeddedLanguageFormatting: "auto"` keeps prettier's default of formatting code inside fenced blocks. The skill does not impose `off` here — a repo whose markdown carries illustrative snippets it does *not* want reformatted (e.g. GitHub-Actions `${{ }}` examples) declares its own `.prettierrc` with `embeddedLanguageFormatting: "off"` to override the fallback.
...
- Don't impose a line *width*. `MD013` is disabled and the bundled prettier config sets `proseWrap: "never"`, so no column limit is enforced. Note that the default formatter (prettier) does **remove** existing hard wraps on format (one line per paragraph) — it never introduces them, and never enforces a width.
