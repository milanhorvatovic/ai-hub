...
**Currency.** The mantras and principles are meant to age well; advice that names a tool or a runtime version is not, and pretending otherwise is how a rulebook ends up recommending an interpreter that went end-of-life two releases ago, or a library whose maintainers shut it down. Five reference files per language carry the month their claims were last checked — `best-practices.md`, `concurrency.md`, `dependencies.md`, `performance.md`, `project-structure.md` — along with the shared references that name tools or platform facts. Their claims take whatever form outlives the month: "the current LTS line" over a version number, a floor (`3.10+`) over a snapshot. The stamp sits at the top of the file, or at the head of the section holding the claims when the rest of the file does not decay, so a reader meets the date before the advice. The other two per-language files carry none by design — `anti-patterns.md` is language semantics and `examples.md` is code — and capability entry points inherit, summarizing references rather than repeating a date that could drift from them.

A test pins that classification in both directions and fails on a reference it has never seen, which is the part that matters: the version of this rule that only described the intent was falsified four times, because an unstamped recommendation reads as a fresh one and nothing was checking. Read a stamped file as accurate to its date and no further: past the stamp a named version is the last one verified, not today's answer, so re-check before repeating it to a user. Version literals inside syntax examples — the pin in a `Cargo.toml` snippet, the dependency in a `package.json` — demonstrate the syntax and are not currency claims.

Re-checking a stamp means enumerating the tools the file names and asking of each whether it still exists, not scanning for version numbers that look old. A discontinued library carries no version to look stale by, so the version-shaped check passes it every time — which is how a dead async runtime survived two sweeps that were looking for aged release numbers.
...
For languages without a capability (Go, Ruby, Java, C/C++, Swift, …), use the core principles plus the repo's declared conventions; propose a new capability if the language recurs — `references/language-expansion.md` sets the threshold behind _recurs_ and the file set a new capability owes on day one. Capabilities extend the core, never override it — a conflict is a bug, so flag it. Write-mode (avoid violations as you author) is the default and lives in this router; review-mode loads from the review capability.
