...
**Currency.** The mantras and principles are meant to age well; advice that names a tool or a runtime version is not, and pretending otherwise is how a rulebook ends up recommending an interpreter that went end-of-life two releases ago, or a library whose maintainers shut it down. A file that *recommends* a named tool or a runtime version carries the month its claims were last checked, and states them in whatever form outlives that month: "the current LTS line" over a version number, a floor (`3.10+`) over a snapshot. The stamp sits at the top of the file, or at the head of the section holding the claims when the rest of the file does not decay — what matters is that a reader meets the date before the advice. A capability entry point that summarizes what its references cover inherits their stamps rather than carrying a second date that can drift from them, and mentioning a tool in passing earns no stamp at all. One date per set of claims, owned by the file that states them in full. Read a stamped file as accurate to its date and no further: past the stamp a named version is the last one verified, not today's answer, so re-check before repeating it to a user. Version literals inside syntax examples — the pin in a `Cargo.toml` snippet, the dependency in a `package.json` — demonstrate the syntax and are not currency claims.
