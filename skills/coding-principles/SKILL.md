...
**Currency.** The mantras and principles are meant to age well; per-language toolchain advice is not, and pretending otherwise is how a rulebook starts recommending an interpreter that went end-of-life two releases ago. Each language capability's `best-practices.md` therefore opens with the month its toolchain claims were last checked, and states them in whatever form outlives that month — "the current LTS line" over a version number, a floor (`3.10+`) over a snapshot. Read a stamped file as accurate to its date and no further: past the stamp a named version is the last one verified, not today's answer, so re-check before repeating it to a user. Version literals inside syntax examples — the pin in a `Cargo.toml` snippet, the dependency in a `package.json` — demonstrate the syntax and are not currency claims.
