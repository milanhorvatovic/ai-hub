...
**Currency.** The mantras and principles are meant to age well; advice that names a tool or a runtime version is not, and pretending otherwise is how a rulebook ends up recommending an interpreter that went end-of-life two releases ago, or a library whose maintainers shut it down. Every reference file that recommends a named tool or a runtime version carries the month its claims were last checked, and states them in whatever form outlives that month: "the current LTS line" over a version number, a floor (`3.10+`) over a snapshot. The stamp sits at the top of the file, or at the head of the section holding the claims when the rest of the file does not decay — what matters is that a reader meets the date before the advice. Capability entry points are the one exception, and inherit: they summarize what their references cover, so a second date there would only drift from the first. Mentioning a tool in passing earns no stamp at all. The set is pinned by a test rather than by this paragraph's good intentions, because a policy that describes more than it enforces teaches readers to treat an unstamped recommendation as fresh. Read a stamped file as accurate to its date and no further: past the stamp a named version is the last one verified, not today's answer, so re-check before repeating it to a user. Version literals inside syntax examples — the pin in a `Cargo.toml` snippet, the dependency in a `package.json` — demonstrate the syntax and are not currency claims.
