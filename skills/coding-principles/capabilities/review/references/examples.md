...
- **The principle tag is a label; the sentence after it leads with the observable thing** — `order.card_number` reaching the logs, the unreachable `is None` branch — rather than restating the rule in its own words. The number is there so a finding is auditable; the prose is there so a reviewer who has never read this skill can act on all six without looking any of them up.
