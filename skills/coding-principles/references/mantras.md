...
- **Observability** — Production code is debuggable without attaching a debugger to it. Logs at the edges (entry, exit, error) with structured fields, not raw string-concat narration of the interior. Errors carry context (what was attempted, how it failed, against what state) — `raise ValueError("invalid")` is worthless; `raise ValueError(f"invalid email: no domain part (user_id={user_id})")` is debuggable. Note what that context is made of: the reason and a bounded id, never the rejected value, which is where observability meets principle 13's limit on what an error may carry. Metrics where load or correctness matters; tracing across async or service boundaries. Like testability, observability is a quality signal of Tier 2 design: if the only way to understand a failure in production is to add a log line and redeploy, the design did not earn its shape.
