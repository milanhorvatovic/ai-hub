...
Some of it also reaches outside the skill directory: a repo packaging several skills may keep a manifest naming their capabilities, and the activation corpus behind the description is a repo-level artifact too. Its contributor docs own that list, so read them rather than assuming the skill directory is the whole job.
...
Go, Java, Swift, Ruby, and C/C++ — the languages the Capabilities section names as uncovered. None is at the threshold today.

The tally lives in the host repo's issue tracker rather than in this list, one proposal per language, because a shipped file is a poor ledger: every sighting would edit the skill, and consumers would install somebody else's running count.
