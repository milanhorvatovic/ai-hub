...
- **Audience scan** — run the same messages through `../../references/publication-audience.md`. A squash body is assembled from several commits' worth of context, which is exactly where a reference to something only the author can open survives the merge into permanent history. On match → a self-contained rewrite at the grade the match carries. A heuristic `WARN` lets the plan proceed, since that finding is about wording rather than safety; a repository-declared `error` holds the plan until the message is fixed, because the repository has said that span must not reach its history.
