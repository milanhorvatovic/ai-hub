...
**Scan whatever the merge policy will actually use, not only text typed here.** Derive the message sources from the method being surfaced and the repository's `sm` / `st` values per `../../references/merge-policy.md`, then run both scans — secret and audience — over every source that will land: the PR body or title where squash settings select them, the branch's commit messages where the squash body is concatenated from them or a rebase preserves them verbatim, the sole commit's subject under a `COMMIT_OR_PR_TITLE` default, and the PR title a merge commit carries. Each of those is the permanent message on some path, and enumerating two of them is how the other five stay unscanned. Fetch what the policy names and scan it before surfacing any command; where the settings query fails, say the sources are unverified rather than assuming the narrow case.
