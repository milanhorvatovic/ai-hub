...
- `git diff --cached --name-only -z` — **the authoritative path list**, and the only read here whose output can be trusted verbatim. Every other form quotes paths and separates them by newline, so a name containing a quote or a newline comes back mangled or split in two, and a partition file built from it names files that do not exist. Group and partition from this list; use the line-oriented reads for churn and content, never for identity.
