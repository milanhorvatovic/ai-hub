...
- Don't auto-execute any force-push, and never suggest bare `git push --force`. When HEAD is pushed, surface the `git push --force-with-lease origin <branch>` recipe via the Step 5 warning, following the surfacing policy in `../../references/force-push-impact.md`; the user runs it.
