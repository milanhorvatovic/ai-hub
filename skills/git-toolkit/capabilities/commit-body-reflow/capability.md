...
The preview displays full bodies, so the Step 7 scans apply here too: run `../../references/secret-patterns.md` over every BEFORE/AFTER pair and redact matched spans per its action steps before emitting, and `../../references/publication-audience.md` over the same pairs — a reflow republishes every line it touches, so an inherited private reference is surfaced now rather than carried through silently. Where a match carries a repository-declared `error`, the span is withheld from **both** halves of the pair — a private reference that predates the reflow sits in BEFORE, and printing it there publishes it exactly as printing it in AFTER would — and the proposed rewrite takes the pair's place: a preview that shows the offending text has shown it, dry run or not.
...
Before emitting, scan every rewritten body against `../../references/secret-patterns.md` and `../../references/publication-audience.md` — the rewrite is the moment to redact and to resolve an inherited private reference, not to re-leak or relay one. Audience findings carry the grade the match assigns: a heuristic `WARN` lists its commits as artifacts without holding the branch, while a repository-declared `error` holds Step 8 for that branch exactly as a secret match does — and holds the body with it. The verdict names the commit and shows the proposed rewrite, never the body carrying the span in either version, because emitting that text in a verdict publishes it just as surely as pushing it would. The redaction handling below is the secret half. On match, apply the catalog's action steps to everything this capability displays: redact the matched span in verdict artifacts and any body excerpt (`[REDACTED: <pattern_name>]`), WARN per pattern, list the offending commits as artifacts, and hold Step 8 for those branches until the user decides how to handle the underlying message. Never emit matched text un-redacted.
