...
5. **Apply command** — `gh pr edit <num> --body-file <path>` with the **resolved PR number explicitly**, never a branch name. Never run `gh pr edit` automatically. When a title rewrite is proposed, write it to its own `mktemp` file and surface a file-based command beside the body one — `gh api repos/{owner}/{repo}/pulls/<num> -X PATCH -F title=@<path>` — because a body-only command silently leaves a flagged title exactly as it was, and a scan whose fix cannot be applied is a scan that reports for nothing. The title never goes inline in the command: an ordinary title containing an apostrophe (`don't drop the retry`) breaks a single-quoted argument, and what follows the break is read by the shell as syntax rather than text. Every proposal this capability emits is already file-based for the body; the title joins it for the same reason. Where an unresolved `error` finding stands against either surface, no apply command for that surface is shown at all: the rewrite is proposed and the command follows once the span is gone, per the shared contract. The title still moves only by the user's hand, which is the standing rule here and is unaffected by having a command to hand them.
