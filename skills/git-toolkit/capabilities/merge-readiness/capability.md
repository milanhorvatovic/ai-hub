...
| **Description in sync** | Run a light version of the `pr-description` SYNC workflow over the fetched body **and title** — both dimensions: do the claims match the diff, and does the text resolve for a public reader (self-containment, reported as `private-context-ref`)? A body can be accurate and still name what only its author can open, and the title needs saying because the delegate scans it while a body-only gate would pass a self-contained body beside a title carrying a private code — into the squash subject, or into permanent history where the body is the commit message | Warn if MINOR-UPDATE; Fail if MAJOR-REWRITE or HANDOFF-TO-WRITE — either dimension, since the delegated verdict is one label |
