...
- **Bitbucket Cloud** → minimal lane: the operations in the adapter's Bitbucket table route through `bkt`, falling back to the table's raw REST calls when `bkt` is absent (a scoped API token either way); operations outside that table refuse with the documented reason.
