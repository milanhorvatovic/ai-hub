...
**Fix**: caught before the commit, this is `commit-message`'s SPLIT mode — it partitions the staged pile, orders the series dependency-first, and authors a message per partition. Caught after, the repair is `git reset --soft HEAD~` back to a staged pile — or `git update-ref -d HEAD` when the offending commit is the root, where `HEAD~` does not resolve and deleting the ref is the same act, leaving the tree staged on a branch that is unborn again; run `git fetch` and then `git branch -r --contains HEAD` **before** deleting — the fetch is not optional, because `force-push-impact.md`'s staleness caveat applies here with the highest stakes in the skill: a stale tracking ref reports a freshly pushed root as unpublished, and acting on that answer deletes it and make the deletion conditional on an empty result, because the deletion takes the reflog with it and no later step can recover the SHA the force-push guard needs. A non-empty result means the root is published: stop there, emit the Force-Push Impact proposal, and delete nothing. Where containment is empty the whole repair stays one proposal — check, delete, invoke — because nothing carries the containment answer into a later run, so the invocation that follows a root deletion is proposal-only by construction rather than by the user remembering — an unconditional deletion would strip the evidence and let the next invocation apply a replacement series over history someone else has — and then **`/git-toolkit commit --split`** over it — soft rather than the default mixed, which would unstage everything and leave a working tree rather than the pile the mode expects, and `--split` because any tracked edit still in the worktree makes that pile look hand-curated to the mode's own curation rule, which would force it to one commit and quietly undo the repair, or `rebase-cleanup` when the commit is not HEAD. Either way the body should justify why a bundled commit makes sense if you decide to keep it bundled.
