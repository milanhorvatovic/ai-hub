...
For a **root commit** the same repair needs a different command and a check before it. `HEAD~` does not resolve, so `git update-ref -d HEAD` is the equivalent act — it leaves the tree staged on a branch that is unborn again. Before deleting anything, run `git fetch` and then `git branch -r --contains HEAD`, and make the deletion conditional on an empty result. The fetch is not optional: `force-push-impact.md`'s staleness caveat applies here with the highest stakes in the skill, because a stale tracking ref reports a freshly pushed root as unpublished and the deletion that follows destroys the evidence that would have contradicted it. A non-empty result means the root is published — stop there, emit the Force-Push Impact proposal, and delete nothing. Where containment is empty, emit the whole repair as one proposal (check, delete, invoke) rather than as steps the user assembles.
