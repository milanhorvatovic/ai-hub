...
| `bitbucket.org` | Bitbucket Cloud | `bkt` with a CLI-free curl fallback — minimal lane, see its section below |
...
- **`bkt`** (Bitbucket Cloud) — community-maintained (avivsinai/bitbucket-cli; Atlassian ships no official Bitbucket CLI), so pin its version in automation. Authenticate with `bkt auth login https://bitbucket.org --kind cloud --username <atlassian-email> --token <api-token>`, using a **scoped API token** created at id.atlassian.com with Bitbucket as the application (general Atlassian tokens won't work; app passwords are dead; OAuth via `--web` is the alternative). Scopes: `read:pullrequest:bitbucket` for PR reads, `write:pullrequest:bitbucket` for edit/merge, `read:repository:bitbucket` for commit statuses. Every command takes `--json` and a built-in `--jq`; `bkt api <path>` is the passthrough for reads without a dedicated command. The raw REST calls in the lane table are the CLI-free fallback — no forge-specific CLI required: same token, `curl`, plus `jq` where a cell filters JSON.
...
| Merge policy | `gh api repos/{o}/{r} --jq '{squash: .allow_squash_merge, …}'` | `glab api projects/:id` piped to `jq '{merge_method, squash_option, squash_commit_template, merge_commit_template}'` — semantics below; both templates are needed, since a merge template expands the description, source branch, or commit messages into the permanent message exactly as the squash one does | the style is chosen at merge time; squash-message shaping is an on-disk template read (below) |
...
| Edit the PR title | `gh api repos/{owner}/{repo}/pulls/<n> -X PATCH -F title=@<path>` — file-based, so a title containing an apostrophe cannot break the command | `glab mr update <n> --title "$(cat <path>)"` — no file flag | `tea pr edit <n> --title "$(cat <path>)"` |
...
| Edit the PR body | `bkt pr edit <n> --description "$(cat <path>)"` — from the file for the same quoting reason as the title row; confirm reviewers survive the first edit you propose (whether bkt guards the underlying PUT's reviewers drop is unverified) | `GET` the PR first, then `PUT $BB/pullrequests/<n>` with `{"title": …, "description": …, "reviewers": <echoed from the GET>}` — a PUT that omits `reviewers` silently drops the PR's reviewer list |
| Edit the PR title | `bkt pr edit <n> --title "$(cat <path>)"` — read the title from the file rather than inlining it, or an apostrophe ends the quoting and the rest is parsed as shell syntax; same unverified reviewers caveat as the body row | the same `GET`-then-`PUT` round trip, changing `title` and echoing `description` and `reviewers` back — one endpoint carries both fields, so a title edit that forgets the rest overwrites them |
| Merge policy | no dedicated command — `bkt api "/repositories/<workspace>/<repo>/refs/branches/<name>" --jq '{merge_strategies, default_merge_strategy}'` | `destination.branch.merge_strategies` and `.default_merge_strategy` on the PR metadata read (also via `GET $BB/refs/branches/<name>`) — read-only, not settable via the API |
