...
| Edit the PR title | `bkt pr edit <n> --title '<text>'` — same unverified reviewers caveat as the body row | the same `GET`-then-`PUT` round trip, changing `title` and echoing `description` and `reviewers` back — one endpoint carries both fields, so a title edit that forgets the rest overwrites them |
