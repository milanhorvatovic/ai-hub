...
1. Reads `git diff --cached` — sees the retry loop, the timeout constant, the import of a backoff helper. SPLIT's partition analysis runs here in Step 2, as it does on every commit: one file, one concern, so the answer is N=1 and the walkthrough never hears about it. That silence is the designed behaviour, not an omission from this example.
...
- **SPLIT mode's series output** — never reached here. The partition analysis itself did run, at Step 2, and returned N=1 — which is why the walkthrough shows one commit and no splitting decision to make, and why the note there says the silence is the designed behaviour rather than an omission. The series form is what a pile mixing the retry fix with, say, an unrelated CI bump would have produced instead — two ordered commits and their staging protocol.
