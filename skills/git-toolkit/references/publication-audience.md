...
- `private_path` — `(?:^|[\s("'\x60=])(?:/|~/|[A-Za-z]:[\\/]|\\\\)\S+` — absolute, drive-letter, UNC, or home-relative paths, which name a filesystem rather than the repository. Any absolute path qualifies, not a list of familiar roots: an enumeration of `/Users`, `/home` and friends promises to catch absolute paths and then lets `/tmp/design.md`, `/root/notes`, and `/srv/build/plan` through, which is a narrower rule wearing a broader sentence. The delimiter class includes a backtick — written `\x60` so the pattern itself survives being quoted as inline code — because inline code is the ordinary way a path reaches a description, and omitting it would miss the most common spelling of the thing being detected. It includes `=` for the same reason one step further out: a path most often appears in a description as the value of a flag or a variable (`--config=/home/dev/plan.md`, `HOME=~/workspace`), and a delimiter list built from prose habits misses every one of those. It still keeps scheme-qualified URLs out, code-formatted ones included, since the slashes in `https://example.com/x` follow a colon and a letter rather than a delimiter. Root-relative markdown link destinations need resolving rather than matching, though: a destination like `/owner/repo/issues/12` written inside a link puts a leading slash straight after an accepted delimiter, and it is a publicly resolvable link on the publishing forge rather than a filesystem path. Read a markdown link's destination as a link first, and only classify it as a path when it resolves as neither a link on this forge nor a file in the tree. The Windows forms are spelled to match what Windows accepts rather than what a POSIX author pictures: a drive path takes either separator (`C:\Users\dev` and `C:/Users/dev` are the same path) and a UNC path opens with two backslashes, so a pattern that took only `C:\` left two ordinary spellings of a private path unflagged. Absolute forms are this entry's whole job; the relative spelling has its own, below.
- `unresolved_relative_path` — `(?:^|[\s("'\x60=])[\w.-]+(?:/[\w.-]+)+\.[A-Za-z0-9]{1,8}\b` — a repository-relative path, which is a candidate and not yet a finding: most of them are ordinary references to files the reader has. Resolve each one against the tree and the diff (`git ls-files --error-unmatch <path>`, then the changed paths) and flag only what resolves in neither, because a path that leads nowhere the reader can go is a private path spelled relatively — a planning archive, a sibling checkout, a directory that exists on one machine. The trailing extension is a deliberate precision trade: it keeps ordinary prose like "and/or" out of the candidate set at the cost of missing a bare directory reference, which a repository that cares can name exactly in its own declaration.
...
`definite_reference` and `session_deixis` find candidates rather than deliver verdicts. After a match, read the sentence it sits in and drop the finding when that sentence carries a reference that **resolves for the publication audience and names the thing the phrase points at** — an issue or pull request in this repository, or a link whose target anyone can open without credentials. So "See #12 for the plan" and "as discussed in #12" are clean once `#12` exists, while a bare "the plan" is not.

The same step clears an antecedent the artifact itself supplies, and it applies to both entries rather than only to the noun phrases. "As noted above, retries stop after three attempts" appeals to nothing outside the text — the note is above, in the same body the reader is holding — and a warning there would be a guard objecting to a document explaining itself. So before flagging, look for the referent in the artifact: a section, an earlier paragraph, a defined term. Only an appeal that leaves the text unanswered is a finding.

Presence is not resolution, and this is where the exemption is easiest to get wrong. A dangling issue number, an unrelated link two clauses away, or a URL on an intranet host all look like references and hand the reader nothing, so none of them clears the finding: verify the target the way the contract asks — the issue resolves and the link's **target** is readable without credentials, which is not the same as its host being reachable, since a private repository lives on a public host — and keep the warning when verification is unavailable rather than reading an unchecked `#N` as proof. A guard that accepts any `#`-shaped token as an antecedent has been talked out of its own rule by punctuation.
...
**Read declarations from the base branch, never from the branch under review.** A declaration is a judge, and a change cannot supply its own: the head tree of a proposed change is untrusted input like any other, so honoring a declaration it introduces would let a contributor add a catch-all pattern, or raise ordinary prose to `error`, and thereby author the findings that grade their own change. Resolve the declaration against the branch being merged into; when the head modifies it, say so in the output preamble — `WARN: declaration modified on the head; base-branch rules applied` — rather than applying it. That is a diagnostic about where the rules came from, not a finding about the text, so it stays out of the REVIEW rule vocabulary: registry ids grade what a target says, and a stream object claiming one for a provenance note would be reporting the run under an id that means something else. This is the same reason `untrusted-content.md` forbids fetched text from deciding a verdict, and it is why even an addition — which sounds harmless — is base-branch-sourced.

"Base branch" needs a definition for the many runs that have no pull request — a commit drafted before one exists, a reflow across local branches, notes cut on a repository with no remote at all. Resolve the trusted source in this order: the base ref of the pull request under review; failing that, the default branch of the publishing remote; failing that, the tracked upstream of the current branch. When none of those resolves, read the working tree and say so — `WARN: declarations read from the working tree; no trusted ref available` — and treat what it says as additive only: its patterns widen the scan, its severities do not raise anything above `WARN`. That posture keeps a local author's declarations useful without letting an unverifiable source hold a proposal hostage, and it never silently drops the built-in heuristics, which run identically whatever the source turns out to be.

A declared expression that does not compile is dropped, not fatal. Validate each one before use; on a failure, skip that entry, keep every other entry and the whole heuristic catalog, and name it in the preamble — `WARN: declaration entry <n> is not a valid expression; skipped`. An unclosed character class in a repository's own declaration is a typo in a config file, and a typo in a config file must not be able to stop every capability that drafts text from producing a proposal.
