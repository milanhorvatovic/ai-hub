...
- `definite_reference` — `(?i)\b(?:the|this|that|our)\s+(?:packet|plan|program|roadmap|audit|tracker|archive|backlog|spec|writeup|doc(?:ument)?|note|session|thread|discussion)s?\b(?![^.\n]*(?:#\d+|https?://))` — a definite noun phrase naming a document or a conversation the reader was never handed. The lookahead is the resolvable case, made executable rather than left to prose: a sentence that carries an issue reference or a link hands the reader the thing it names. Skip the finding too when the noun is defined earlier in the same text — "the plan below" resolves, and only reading the surrounding text can tell.
- `session_deixis` — `(?i)\b(?:as (?:discussed|agreed|decided|noted|mentioned)|we (?:agreed|decided|discussed)|per (?:the|our) (?:plan|packet|program|roadmap|audit|tracker|archive|backlog|spec|writeup|doc(?:ument)?|note|session|thread|discussion)s?)\b(?![^.\n]*(?:#\d+|https?://))` — an appeal to a conversation the reader was not in. The decision may be worth publishing; the appeal is not, because it cites an inaccessible authority in place of the reasoning. Two narrowings keep the mandatory warning honest: `per the …` takes the same private-document nouns as the entry above rather than any word, so "per the diff" and "per the API docs" are left alone, and the trailing lookahead drops the finding when the same sentence carries an issue reference or a link — "as discussed in #12" points at something the reader can open, which is the contract being met rather than broken.
...
- `private_path` — `(?:^|[\s("'\x60])(?:/|~/|[A-Za-z]:\\)\S+` — absolute, drive-letter, or home-relative paths, which name a filesystem rather than the repository. Any absolute path qualifies, not a list of familiar roots: an enumeration of `/Users`, `/home` and friends promises to catch absolute paths and then lets `/tmp/design.md`, `/root/notes`, and `/srv/build/plan` through, which is a narrower rule wearing a broader sentence. The delimiter class includes a backtick — written `\x60` so the pattern itself survives being quoted as inline code — because inline code is the ordinary way a path reaches a description, and omitting it would miss the most common spelling of the thing being detected. It still keeps URLs out, code-formatted ones included, since the slashes in `https://example.com/x` follow a colon and a letter rather than a delimiter. Repo-relative paths join the finding when they resolve in neither the tree nor the diff (`git ls-files --error-unmatch <path>`), because a path that resolves nowhere the reader can reach is a private path spelled relatively.
