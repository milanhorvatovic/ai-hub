...
| Automation identity | Per-run App tokens via `actions/create-github-app-token` (PKCS#8 keys), split into a release App and a routine-automation App by blast radius; no standing PATs. Decision records in `docs/adr/`. |
