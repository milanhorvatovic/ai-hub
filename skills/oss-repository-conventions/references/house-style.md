...
| Automation identity | Per-run App tokens via `actions/create-github-app-token` (keys stored exactly as GitHub's downloaded PEM), split into a release App and a routine-automation App by blast radius; no standing PATs. Decision records in `docs/adr/`. |
...
| Action pinning | Full-SHA pins with a trailing `# vX.Y.Z` comment, enforced by a base-branch-judged PR gate. |
