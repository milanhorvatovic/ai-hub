...
| --- | --- | --- | --- | --- | --- |
| **Default `GITHUB_TOKEN`** | the workflow's repo, per-job `permissions:` | Not unattended — its pushes trigger nothing; a token-authored PR's runs wait for maintainer approval; only explicit `workflow_dispatch`/`repository_dispatch` run freely | Only with the Actions-can-approve setting on, and not a PR it authored; never code-owner | auto, per-run | `github-actions[bot]` |
...
- **Default `GITHUB_TOKEN` first** — least privilege, no secret to manage. Only reach past it for the things it can't do: it **can't approve PRs** unless the repo's "Allow GitHub Actions to create and approve pull requests" setting is on (off by default — and even on, never code-owner review, and never a PR it authored itself), and its own events **don't cascade unattended** — a token-authored push kicks off no downstream workflow, and where a token-authored PR open/update does create a run it waits for maintainer approval before it executes, while an explicit `workflow_dispatch` / `repository_dispatch` runs freely. So a _hands-off_ downstream trigger is what needs a non-default identity.
- **To make a bot's event cascade** (its push/PR must trigger a downstream workflow), **to add a PR to a merge queue** (the built-in token can't enqueue), **or for truthful attribution** (the autonomy ladder's L2+): a **GitHub App** installation token (preferred) or a tightly-scoped **fine-grained PAT**, never a classic PAT. Plain PR approval alone does not need this — the default token approves behind the Actions-can-approve setting, **as long as the approver is a different identity than the PR author**: a bot cannot approve its own PR, and that setting does not bypass the self-approval rule, so a PR opened _by_ `github-actions[bot]` needs a distinct App or PAT to approve it. **Code-owner review is _not_ one of these needs** — no App can satisfy it (see the section below); that resolves to a code-owner PAT or a reshaped ruleset, a separate decision.
...
- **A code-owner PAT** — a fine-grained PAT belonging to a listed code owner, used by the automation to post the approval. Works where classic bot approvals can't (it is a human identity, so it can be a code owner) — but it is still bound by the same rules: it cannot approve a PR that identity authored, nor satisfy a latest-push-approval rule after making the latest push, so the automation must not reuse one human identity to both create/update and approve the same PR. It also costs a standing human-shaped credential (expiry-driven rotation, leaves with its owner) and approvals that read as a human review no human performed.
...
- This file is the rationale behind the autonomy ladder's `autonomy-scoped-identity` guardrail (`pr-autonomy`): approval/merge running as an over-scoped PAT — or as the default token where the flow needs an App (cascade, attribution) or would self-approve — fails it; a plain, distinct-author default-token approval passes.
