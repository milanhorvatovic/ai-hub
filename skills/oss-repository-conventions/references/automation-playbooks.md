...
5. **Verify** — open an eligible PR and confirm it's approved and auto-merges on green; open a hard-stop PR (major / security / a human-authored CI edit, or a bump of an action a post-merge privileged job runs) and confirm it's held for a human; for an action a PR-triggered privileged job runs, confirm instead that it is barred from bot updates or the job loads from a trusted ref, since a hold can't protect a pin that runs pre-review; confirm an unrecoverable automation state fails the run **red** (not warn-and-pass) so failures surface.
