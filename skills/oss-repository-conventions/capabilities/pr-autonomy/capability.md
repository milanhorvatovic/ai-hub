...
- **Scoped identity** — a least-privilege App token (the default `GITHUB_TOKEN`'s own events don't cascade to the downstream required checks — only an explicit dispatch does — and it approves only when the off-by-default Actions-can-approve setting is on, never code-owner review). Choose the identity per `../../references/automation-identity.md` (App token preferred over a fine-grained PAT; never a classic PAT).
