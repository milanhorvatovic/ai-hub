...
- **Hard stops** — major bumps, security-flagged PRs, breaking changes, and _human-authored_ edits to CI/release/secret paths always require a human. A dependency bot's bump of an action is a weaker case, but not because it "changes no logic" — a new SHA is new code, and the pin makes the revision immutable, not vetted. What bounds the risk is the action's **trust boundary**: an action that runs only in a read-only job with no secrets in scope can do little with a compromised revision, so its bump may be eligible at the repo's tolerance; an action that runs in a secret-bearing or write-scoped job is high-risk, and _how_ to mitigate depends on _when_ that job runs. If it is **PR-triggered**, the bumped pin executes during checks **before** any review — a human hold protects nothing, since the new code already ran with the secrets — so the action must be barred from bot updates or the job's definition taken from a trusted ref (the ci-automation `fork-pr-safe` rule; the actions the policy job itself runs are the sharpest instance). If the privileged job runs only **post-merge or off a non-`pull_request` trigger**, it cannot execute the new pin before merge, so holding the bump for a human review is enough. Either way a repo with a demonstrably equivalent control scores clean (recommend, not mandate). The audit scores whether a **privileged-context** action update can reach merge with no human review _and_ no equivalent control — not whether every workflow-file touch does.
- **Scoped identity** — least-privilege. The default `GITHUB_TOKEN` is the floor and is enough when the flow only needs a plain approval (Actions-can-approve setting on, `pull-requests: write`), no bot event has to cascade, and **the PR was authored by a different identity than the approver** — a bot can't approve its own PR, so a PR opened by `github-actions[bot]` needs a distinct App/PAT to approve it (a Dependabot PR is fine: its author and the approver differ). Move up to a **GitHub App** token when an authored event must trigger a downstream required check, when the flow enqueues to a **merge queue** (the built-in token can't), or for truthful bot attribution; a fine-grained PAT is the fallback, never a classic PAT. Code-owner review is a _separate_ remedy — no App can satisfy it, so it resolves to a code-owner PAT or a reshaped ruleset. Choose per `../../references/automation-identity.md`.
...
- L2+ needs `gh` and an approving identity — the default `GITHUB_TOKEN` suffices for a plain review count (Actions-can-approve on) **when it did not author the PR** (a bot can't approve its own PR; detect the PR author and require a distinct App/PAT for self-authored automation PRs), an App only when an event must cascade or for attribution; L3+ needs branch protection with required checks. Detect these; if absent, scaffold them as prerequisites (settings are proposed, not applied). The full provisioning surface each rung adds — the approving identity, the Actions/Dependabot secret stores, gating labels, the auto-merge repo settings, and code-owner review satisfied by either a code-owner identity or a reshaped ruleset — is catalogued in `../../references/automation-prerequisites.md` (its rung table mirrors this ladder); whether it's all in place is the cross-cutting `automation-prereqs-provisioned` check owned by the automation-baseline capability, not re-scored here.
...
- `autonomy-hard-stops` — **should** (when above L1). Fail when major/security/breaking changes, or human-authored CI/release/secret edits, can complete without a human's approval, or when a hard-stop label applied after arming can't disarm an already-armed auto-merge. (A bot's bump of an action that runs in no secret-bearing or write-scoped job is a repo risk call. One that runs in a **PR-triggered** privileged job cannot be fixed by a human hold — the pin runs before review — so fail it unless the action is barred from bot updates or the job loads from a trusted ref; one that runs only **post-merge** may be held for review. See the guardrail above.) Armed-but-never-approved is a compliant hold — the automation may arm these, so long as only a human review can complete them, including late.
- `autonomy-scoped-identity` — **should** (when above L1). Fail when approval/merge runs as an over-scoped PAT, or as the default token **where the flow needs what the default token can't do** — a bot action whose event must cascade to a downstream required check, an enqueue to a merge queue (the built-in token can't), or code-owner review. A minimally-scoped default token approving a PR it did **not** author, to satisfy a _plain_ review count (Actions-can-approve on, `pull-requests: write`) with no required cascade, is itself the least-privilege choice and scores clean — but fail it when the default token would approve a PR authored by `github-actions[bot]` (a bot can't approve its own PR, so that flow is non-functional and needs a distinct App/PAT). The App is preferred for cascade and truthful attribution; **code-owner review is not an App remedy** — no App can satisfy it, so that resolves to a code-owner PAT or a reshaped ruleset (`../../references/automation-identity.md`).
...
- Don't auto-approve/merge with an over-scoped PAT, or with the default `GITHUB_TOKEN` where the flow needs an App (an event that must cascade, an enqueue to a merge queue, truthful attribution) or where the token would approve its own PR — a plain, distinct-author approval with a direct squash-merge on the default token is fine.
