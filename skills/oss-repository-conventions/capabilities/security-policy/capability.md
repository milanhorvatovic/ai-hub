...
- `code-scanning` — **should** (when the repo ships code in a language supported by CodeQL or an equivalent SAST) · scorecard: SAST. Fail when no static-analysis workflow runs in CI — CodeQL (`github/codeql-action`) or an equivalent SAST (Semgrep, SonarCloud). A language no scanner covers puts the check out of scope — skip it rather than fabricate a scanner (see the automation-baseline capability's language notes). Static analysis catches vulnerability patterns review misses. This is the code-scanning pillar of the automation baseline, scored here; automation-baseline scaffolds the CodeQL workflow and its roll-up cites this check.
