...
`CODE_OF_CONDUCT.md` — write after confirmation from `references/scaffold-templates.md` (Contributor Covenant). Replace the contact placeholder with the maintainer-provided enforcement contact before writing; do not write the file with a placeholder left in. The house default places the file at repo root (`../../references/house-style.md`); GitHub also surfaces it from `.github/`.
