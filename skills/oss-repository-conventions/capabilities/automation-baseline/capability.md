...
Building blocks live in `references/scaffold-templates.md` (setup composite action, thin `ci.yml` caller with `static`/`test`/`coverage` jobs, CodeQL workflow) — these are the artifacts for the CI-baseline flow. Write after confirmation, tailored to the stack. Then point the maintainer at the pillar recipes for the rest of the baseline:
