...
For hands-off updates, scaffold the **autonomous Dependabot recipe** in the same file (release-label → auto-merge → reconciler), distilled from a proven setup. The recipe classifies every bot PR into one of three tiers — **eligible** (patch/minor of an unprivileged dependency: approve + arm auto-merge, hands-off), **held** (major, or a privileged dependency: arm but never approve, so one human review completes the merge), and **veto** (a hard-stop label: untouched, disarmed if already armed, and any pre-veto bot approval dismissed) — with the pr-autonomy failure postures (a step that grants autonomy denies on failure; a step that revokes it blocks on failure, loudly, behind a required context). Holding a privileged-dependency update for one review is enough only when that dependency does not _execute with privilege before the review_. An action run by any **PR-triggered secret-bearing or write-scoped job** — not only the policy job's own metadata and token-minting actions — runs its bumped pin from the PR head before the tier decision, credentials in scope, so a held review cannot protect it: bar every such action from bot updates in the update-bot config (their pins arrive as human PRs), or load that job from a trusted-ref definition (the ci-automation `fork-pr-safe` check states the general rule). The held tier's one-review completion applies to the rest — majors, and privileged dependencies that only run post-merge. It requires:

- a **GitHub App token or bot PAT** — the default `GITHUB_TOKEN`'s own events don't cascade to the downstream required checks _unattended_ (a token-authored PR's runs, where they exist, wait for approval) and it approves only behind an off-by-default setting; pick per `../../references/automation-identity.md` (App token preferred);
...
- an **update-type gate** (`dependabot/fetch-metadata`) that auto-merges patch/minor, **holds** major and privileged-dependency PRs for one human review (arm-never-approve), and treats a **security-review label as a veto** — disarm and dismiss any bot approval, not a held PR that one review completes;
