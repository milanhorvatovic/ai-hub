...
- A **GitHub App token** (`actions/create-github-app-token`, App-only for the review-dismissal filters). The default `GITHUB_TOKEN` can satisfy a plain required-review count (with the Actions-can-approve setting on and `pull-requests: write`) but never code-owner review — the reasons to prefer the App here are the cascade (its own events don't trigger the downstream required checks; only explicit dispatch does) and the paginated review-dismissal, neither of which the default token does. Stand it up per the capability's automation-prerequisites reference (dual secret stores, PKCS-agnostic key handling, verify with an actored run).
