...
- `actions-pinned` — **should** · scorecard: Pinned-Dependencies. Fail when third-party actions use a moving tag/branch instead of a full commit SHA. A retagged action can inject code. Recommend the human half ride the same line as a **trailing** version comment — the trailing _position_ is the property that matters, since dependency bots maintain a same-line comment when they bump the pin while a comment on the line above rots silently. Either a major channel (`# v4`, as the scaffold templates use) or an exact release (`# v4.2.1`) is fine; the bot keeps whichever form is there.
...
- `workflow-invariants-pinned` — **could**. Pass when the repo's workflow invariants — every runner-backed job carries a timeout (a job that calls a reusable workflow cannot declare one; its cap lives in the callee), permissions stay on the read-only floor with per-job elevation, pins hold their format, and trust-boundary guard conditions exist where the fork-PR check demands them — are held by an **automated gate** that fails a change widening them, so it can't slip through on review alone. What scores clean is the enforcement, not its shape: a repo test suite that parses the workflows, an organization ruleset, or a policy-as-code check (OPA/Conftest and the like) all satisfy it. The house-style _recommendation_ is a repo test suite (it travels with the code and runs on every PR), but audit scores the risk — invariants held only by human vigilance, eroding one approved edit at a time — not the fleet's particular answer to it.

## Scaffold

The base composable CI shape (setup action, thin caller, CodeQL) comes from the automation-baseline capability; this capability _hardens_ it. For the ordered end-to-end hardening pass (flow 3), follow `../../references/automation-playbooks.md`. Apply the patterns in `references/scaffold-templates.md`: default `permissions: contents: read` and elevate per job, pin every third-party action to a commit SHA with a trailing `# vX.Y.Z` comment, use OIDC for deploy/publish, and set concurrency cancellation and job timeouts. Scaffold the optional OpenSSF Scorecard workflow from the same file. When a convention deserves a PR gate (pin format, permission floors), the trustworthy shape runs the checker on `pull_request_target`, whose workflow definition GitHub loads from the repository's **default branch — never the PR head** — so a PR can't edit its own judge; it fetches only the specific head files it statically parses, as data, never a checkout into the workspace, never executed, per this capability's fork-PR guard. Because the judge always comes from the default branch, the gate is only active once it lives there: a gate merged only to a maintenance/release branch never runs, and the PR _introducing_ the gate goes unjudged by it — the first real run lands on the next PR after the merge. Modify workflows only on confirmation, showing a diff for existing files.
