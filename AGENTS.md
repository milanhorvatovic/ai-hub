...
- **Automation identity:** which identity a workflow acts as is a decision, not a default — a `GITHUB_TOKEN`-authored push triggers no downstream workflow, and a GitHub App cannot satisfy a code-owner review. Before elevating a workflow's `permissions:`, or adding a secret, read `docs/adr/0002-automation-identity.md`. Two tests hold the audit to the tree — one pins which jobs elevate the default token, the other pairs every secret a workflow reads with the audit row that names it — so either kind of new identity is a deliberate edit rather than a quiet one.
