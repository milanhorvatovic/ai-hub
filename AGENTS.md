...
- **Automation identity:** which identity a workflow acts as is a decision, not a default — a `GITHUB_TOKEN`-authored push triggers no downstream workflow, and a GitHub App cannot satisfy a code-owner review. Before giving a workflow write access to `contents` or `pull-requests`, or adding a secret, read `docs/adr/0002-automation-identity.md`; a test pins the set of jobs that may author events, so widening it is a deliberate edit.
