...
- **The release PR is the live instance of the first rule.** [#45](https://github.com/milanhorvatovic/ai-hub/pull/45) has been open since 2026-07-13 carrying one status against nine required contexts, because release-please pushes it with the default token — `token: ${{ secrets.GITHUB_TOKEN }}` in `release-please.yml`. The credential that pushed is what decides, not the author recorded on the commit: `github-actions[bot]` in the author column is a symptom, and the same commit would start runs had an App or a PAT pushed it. Debugging a PR with no checks therefore means asking which token pushed, not who the commit says wrote it. The single reporter is an app-posted check, which is not subject to the suppression rule; nothing that could gate the release ever starts. It is mergeable only through the ruleset's admin bypass, so the first release of this catalog would publish with no CI having run on it.
