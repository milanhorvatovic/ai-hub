...
- **Least privilege stays explicit.** Each App is scoped to Contents and Pull requests write, no standing credential exists at all, and every workflow keeps its read-only `permissions:` floor — the only jobs elevating the default token are the two that upload release artifacts.
